...
import { LogOut, Calendar, Clock, CheckCircle, XCircle, Plus, Bell, Users, Zap, AlertTriangle } from 'lucide-react'
...
export default function TeamLeadDashboard({ user, onLogout }: any) {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'shifts' | 'pending' | 'timeoff'>('overview')
  ...
  const approveShift = async (shiftId: number) => {
    try {
      const response = await fetch(`/api/shifts/${shiftId}/approve-teamlead`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${localStorage.getItem('token')}`
      ...
      if (response.ok) {
        fetchShifts()
        fetchPendingClaims()
        alert('Shift approved! Pending final manager approval.')
      }
    } catch (error) {
  ...
  const approveTimeOff = async (requestId: number) => {
    try {
      const response = await fetch(`/api/timeoff/${requestId}/approve-teamlead`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${localStorage.getItem('token')}`
      ...
      if (response.ok) {
        fetchTimeOffRequests()
        alert('Time off approved! Pending final manager approval.')
      }
    } catch (error) {
      ...
      <header className="bg-gradient-to-r from-blue-700 to-blue-600 dark:from-blue-900 dark:to-blue-800 text-white shadow-lg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-3xl font-bold">ClockWork - Team Lead</h1>
              <p className="text-blue-200 dark:text-blue-300 mt-1">Welcome, {user.name}</p>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600 dark:text-gray-400">My Employees</p>
                <p className="text-3xl font-bold text-blue-600 dark:text-blue-400">{stats.myEmployees}</p>
              </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600 dark:text-gray-400">Team Shifts</p>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600 dark:text-gray-400">Time Off Requests</p>
                <p className="text-3xl font-bold text-purple-600 dark:text-purple-400">{stats.pendingTimeOff}</p>
              </div>
              <AlertTriangle className="w-12 h-12 text-purple-400 dark:text-purple-500" />
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-4 mb-6 border-b border-gray-200 dark:border-gray-700 overflow-x-auto">
          <button
            ...
            className={`pb-3 px-4 font-medium transition flex-shrink-0 ${
              activeTab === 'overview'
                ? 'border-b-2 border-blue-600 dark:border-blue-500 text-blue-700 dark:text-blue-400'
        ...
        <div className="space-y-6">
          {activeTab === 'overview' && (
            <>
              <div className="flex justify-between items-center">
                <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-200">Team Overview</h2>
                <button
                  onClick={() => setShowCreateModal(true)}
                  className="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 dark:bg-blue-700 dark:hover:bg-blue-600 text-white rounded-lg transition"
                >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
                  <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-4">My Team Members</h3>
                  {employees.length === 0 ? (
                    <p className="text-gray-500 dark:text-gray-400">No employees assigned yet</p>
                  ) : (
                    <div className="space-y-3">
                      {employees.map(emp => (
                        <div key={emp.id} className="p-3 bg-gray-50 dark:bg-gray-700 rounded">
                      ...
                      <div key={claim.id} className="flex items-center gap-3 p-3 bg-yellow-50 dark:bg-yellow-900/30 rounded">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-gray-600 dark:text-gray-400">
                            {format(parseISO(claim.date), 'MMM dd')} - Pending approval
                          </p>
                        </div>
                      </div>
                    ))}
                    {pendingClaims.length === 0 && (
                      <p className="text-gray-500 dark:text-gray-400 text-sm">No pending items</p>
                    )}
                  </div>
                </div>
              </div>
          ...
          {activeTab === 'shifts' && (
            <>
              <div className="flex justify-between items-center">
                <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-200">Team Shifts</h2>
                <button
                  onClick={() => setShowCreateModal(true)}
                  className="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 dark:bg-blue-700 dark:hover:bg-blue-600 text-white rounded-lg transition"
                >
                        ...
                        <div>
                          {request.status === 'pending' ? (
                            ...
                            <span className={`px-3 py-1 rounded-full text-sm ${
                              ...
                              request.status === 'pending-manager' ? 'bg-blue-100 dark:bg-blue-900/40 text-blue-800 dark:text-blue-300' :
                              'bg-red-100 dark:bg-red-900/40 text-red-800 dark:text-red-300'
                            }`}>
                              {request.status === 'pending-manager' ? 'Awaiting Manager' : request.status}
                            </span>
                          )}
                        </div>
                      </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full p-6">
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3">
                <button
                  onClick={createShift}
                  className="flex-1 px-4 py-2 bg-blue-600 hover:bg-blue-700 dark:bg-blue-700 dark:hover:bg-blue-600 text-white rounded-lg transition"
                >
                  Create Shift
                </button>
                <button
